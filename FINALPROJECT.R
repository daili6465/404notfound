...
View(Pet_Data)
library(dplyr)
Only_Date <- select(Pet_Data, License.Issue.Date, Species)
View(Only_Date)
library(lubridate)
library(ggplot2)
library(tidyr)

#Making month/ year stuff
...
Population_Dog <- rbind.data.frame(D2004, D2008, D2011, D2012, D2014,
...
View(Population_Pets)
# Create the plot
Pet_Plot <- ggplot(Population_Pets, aes(Year, Population, size = Population, color = Species, frame = Month)) + geom_point()+ 
  labs(x="Year", y = "Population of Pets Licenses", color = 'Species',size = "Population") + 
  scale_color_brewer(type = 'div', palette = 'Spectral')

Animation <- ggplotly(Pet_Plot)

Animation
htmlwidgets::saveWidget(Animation, "Plot.html")